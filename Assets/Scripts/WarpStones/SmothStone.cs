...
public class SmothWarpStone {
    ...
    private Vector3[] SetVertices()
    {
        ...
        Vector3 vertexA = this.baseWarpStone.GetPointOnTorus(warpSettings.depthIndex * uStep, warpSettings.startIndex * vStep);
        Vector3 vertexB = this.baseWarpStone.GetPointOnTorus((warpSettings.depthIndex + warpSettings.depthFactor) * uStep, warpSettings.startIndex * vStep);

        Vector3 endPoint = this.baseWarpStone.CreateTriangleEndpoint((warpSettings.depthIndex) * (uStep / 2), (warpSettings.startIndex + warpSettings.warpStoneFactor) * (vStep / 2));
        int verticeIndex = 0;
        for(int sideStep = 1; sideStep <= warpSettings.warpStoneFactor; sideStep++)
        {
            // Front side
            vertices[verticeIndex] = vertexA;
            vertices[verticeIndex + 1] = vertexA = this.baseWarpStone.GetPointOnTorus(warpSettings.depthIndex * uStep, (warpSettings.startIndex + sideStep) * vStep);
            vertices[verticeIndex + 2] = endPoint;
            // Back side
            vertices[verticeIndex + 3] = vertexB;
            vertices[verticeIndex + 4] = vertexB = this.baseWarpStone.GetPointOnTorus((warpSettings.depthIndex + warpSettings.depthFactor) * uStep, (warpSettings.startIndex + sideStep) * vStep);
